...

{
	...
	if (m_ov.hEvent != NULL)
		...
		m_ov.hEvent = CreateEventW(NULL, TRUE, FALSE, NULL);
	...
	*/
	m_hComm = CreateFile(szPort,						// communication port string (COMX)
		...
		FILE_FLAG_OVERLAPPED,			// Async I/O   FILE_FLAG_OVERLAPPED
	...
	for (;;)
	{
		...
		*/
		...
		if (!bResult)
		{
			...
			{
										  port->m_hComm = INVALID_HANDLE_VALUE;
										  ::SendMessage(port->m_pOwner, WM_COMM_ERROR_INVALID_HANDLE, (WPARAM)1, (LPARAM)port->m_nPortNr);
										  break;
			}
